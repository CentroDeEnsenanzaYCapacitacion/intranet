...
@section('content')
    ...
    @if (session('success'))
        <div id="success-alert" class="alert alert-success" role="alert" style="background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); border-left: 4px solid #10b981; border-radius: 12px; padding: 16px; margin-bottom: 24px;">
            {{ session('success') }}
        </div>
    @endif
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="table-actions">
                                ...
                                <form action="{{ route('admin.staff.deactivate', $person->id) }}" method="POST" class="d-inline" data-password-confirm>
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="action-btn action-delete" title="Desactivar">
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
