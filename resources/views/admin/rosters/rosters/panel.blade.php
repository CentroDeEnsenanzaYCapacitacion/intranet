        ...
        <div class="card-body">
            ...
            12 => 'Diciembre',
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-control" name="period" id="period">
                            <option value="8-22" {{ request('period') == '8-22' ? 'selected' : '' }}>Del 8 al 22</option>
                            <option value="23-6" {{ request('period') == '23-6' ? 'selected' : '' }}>Del 23 al 6</option>
                        </select>
                    </div>
                </div>
            ...
            @if (!empty($staffGrouped))
                @foreach ($staffGrouped as $crewId => $staffList)
                    ...
                    <p class="text-muted mb-1">Horas trabajadas en este plantel: <strong>{{ $crewHours }}</strong></p>
                    ...
                    <ul>
                        ...
                        @empty
                            <li>No hay trabajadores en este plantel.</li>
                        @endforelse
                    </ul>
                @endforeach
            @else
                <p class="text-muted">No hay trabajadores para mostrar.</p>
            @endif

            @if (!is_null($totalHours))
                <div class="alert alert-info mt-4">
                    Total de horas trabajadas en el periodo seleccionado: <strong>{{ number_format($totalHours, 1) }}</strong>
                </div>
            @endif
        </div>
    </div>
